...
def criar_conta():
    ...
    comando = f'INSERT INTO pessoas (nome, email, senha) VALUES ("{nome}", "{email}", "{senha_criptografada}")'
    try:
        ex_comando("POST", comando)
...
def fazer_login():
    ...
    comando= f'SELECT id, senha FROM pessoas WHERE email = "{email}" LIMIT 1'
    retorno= ex_comando("GET_BY_ID", comando)

    #se tiver retorno, essas duas variáveis recebem o retorno
    if retorno:
        id_usuario_db, senha_criptografada_db = retorno
...
def mostrar_sustentabilidade(id_usuario):
    ...
    comando = f"SELECT media_final, classificacao_final FROM resultados_sustentabilidade WHERE pessoa_id = {id_usuario} ORDER BY id DESC LIMIT 1"
    resultado = ex_comando("GET_BY_ID", comando)
...
def mostrar_graficos(id_usuario):
    ...
    resultado = ex_comando("GET", comando)
...
def enviar_dados(id_usuario):
    ...
    comando_inserir = f"""
        INSERT INTO monitoramento_parametros (
            ...
            {id_usuario}, CURDATE(), {agua}, NULL, {energia}, NULL, {residuo}, '{transporte}', {distancia}
        )
    """
    ...
    comando_update_resultado = f"""
        ...
        SET 
            ...
            classificacao_final = CASE
                WHEN media_final >= 2.5 THEN 'Sustentável'
                WHEN media_final >= 1.5 THEN 'Mediano'
                ELSE 'Não Sustentável'
            END
        WHERE pessoa_id = {id_usuario} AND data_calculo = '{data_registro}';
    """
